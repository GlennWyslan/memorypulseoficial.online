function runDelayedFunctions(data) {
  try {
    document
      .querySelectorAll(".atomicat-delay")
      .forEach((el) => el.classList.remove("atomicat-delay"));
...
(function () {
  try {
    const clickeventList = [
      { compKey: "4cfd05d", misc: { type: "button" } },
      { compKey: "ba9a403", misc: { type: "text" } },
      { compKey: "01075ae", misc: { type: "button" } },
      { compKey: "90e4c62", misc: { type: "text" } },
      { compKey: "cd2ae25", misc: { type: "button" } },
      { compKey: "f2cf3d7", misc: { type: "text" } },
      { compKey: "c332c43", misc: { type: "text" } },
    ];
...
(function () {
  try {
    const animationList = [
      { key: "4cfd05d", type: "button" },
      { key: "01075ae", type: "button" },
      { key: "cd2ae25", type: "button" },
    ];
    ...
    animationList.forEach((animationItem, index) => {
      ...
      const elementClass =
        type === "container"
          ? ".atomicat-container-" + key
          : ".atomicat-element-container-" + key;
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            targetElement.classList.remove(
              "atomicat-entrance-animation-" + key
            );
...
(function () {
  try {
    ...
    var SECONDS_TO_DISPLAY = 100;
    var attempts = 0;
    var elsDisplayed = false;
    var alreadyDisplayedKey = "alreadyElsDisplayed100";
    ...
    function startWatchVideoProgress() {
      ...
      videoInstance.on("timeupdate", () => {
        ...
        console.log(
          "currentTime => " +
            videoInstance.video.currentTime +
            " SECONDS_TO_DISPLAY => " +
            SECONDS_TO_DISPLAY
        );
